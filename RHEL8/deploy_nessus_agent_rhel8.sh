...
DOWNLOAD_LINK="https://www.tenable.com/downloads/api/v1/public/pages/nessus-agents/downloads/24583/download?i_agree_to_tenable_license_agreement=true"
...
if [ -d "${NESSUS_AGENT_DIR}" ]; then
    ...
    dnf remove NessusAgent -y  # Assuming the package name is NessusAgent; adjust if necessary
...
TEMP_FILE=$(mktemp)
if dnf install -y "${RPM_FILE}" &> "$TEMP_FILE"; then
